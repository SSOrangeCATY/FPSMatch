...
public interface AmmoBoxItemDataAccessor extends IAmmoBox {
    ...
    default Identifier getAmmoId(ItemStack ammoBox) {
        CompoundTag tag = ItemStackNbtHelper.getTag(ammoBox);
        if (ItemStackNbtHelper.contains(tag, AMMO_ID_TAG, Tag.TAG_STRING)) {
            return Identifier.parse(ItemStackNbtHelper.getString(tag, AMMO_ID_TAG));
        }
    ...
    default void setAmmoId(ItemStack ammoBox, Identifier ammoId) {
        ItemStackNbtHelper.updateTag(ammoBox, tag -> tag.putString(AMMO_ID_TAG, ammoId.toString()));
    }
    ...
    default int getAmmoCount(ItemStack ammoBox) {
        ...
        if (ItemStackNbtHelper.contains(tag, AMMO_COUNT_TAG, Tag.TAG_INT)) {
            return ItemStackNbtHelper.getInt(tag, AMMO_COUNT_TAG);
        }
        return 0;
    }
    ...
    default void setAmmoCount(ItemStack ammoBox, int count) {
        ItemStackNbtHelper.updateTag(ammoBox, tag -> tag.putInt(AMMO_COUNT_TAG, isCreative(ammoBox) ? Integer.MAX_VALUE : count));
    }
    ...
    default ItemStack setAmmoLevel(ItemStack ammoBox, int level) {
        ItemStackNbtHelper.updateTag(ammoBox, tag -> tag.putInt(LEVEL_TAG, Math.max(level, 0)));
        return ammoBox;
    }
    ...
    default int getAmmoLevel(ItemStack ammoBox) {
        CompoundTag tag = ItemStackNbtHelper.getTag(ammoBox);
        if (ItemStackNbtHelper.contains(tag, LEVEL_TAG, Tag.TAG_INT)) {
            return ItemStackNbtHelper.getInt(tag, LEVEL_TAG);
        }
        return 0;
    }
    ...
    default boolean isCreative(ItemStack ammoBox) {
        CompoundTag tag = ItemStackNbtHelper.getTag(ammoBox);
        if (ItemStackNbtHelper.contains(tag, CREATIVE_TAG, Tag.TAG_BYTE)) {
            return ItemStackNbtHelper.getBoolean(tag, CREATIVE_TAG);
        }
        return false;
    }
    ...
    default boolean isAllTypeCreative(ItemStack ammoBox) {
        CompoundTag tag = ItemStackNbtHelper.getTag(ammoBox);
        if (ItemStackNbtHelper.contains(tag, ALL_TYPE_CREATIVE_TAG, Tag.TAG_BYTE)) {
            return ItemStackNbtHelper.getBoolean(tag, ALL_TYPE_CREATIVE_TAG);
        }
        return false;
    }
    ...
    default ItemStack setCreative(ItemStack ammoBox, boolean isAllType) {
        ItemStackNbtHelper.updateTag(ammoBox, tag -> {
            if (isAllType) {
                // 移除可能存在的创造模式标签

...
public record SpectatorSwitchC2SPacket(SpectatorSwitchDirection direction) {
    public static void encode(SpectatorSwitchC2SPacket p, FriendlyByteBuf b) { b.writeEnum(p.direction()); }
    public static SpectatorSwitchC2SPacket decode(FriendlyByteBuf b) { return new SpectatorSwitchC2SPacket(b.readEnum(SpectatorSwitchDirection.class)); }
    public void handle(Supplier<NetworkEvent.Context> s) {
        NetworkEvent.Context c = s.get();
        c.enqueueWork(() -> { ServerPlayer p = c.getSender(); if (p != null && p.isSpectator()) MinecraftForge.EVENT_BUS.post(new SpectatorSwitchInputEvent(p, direction)); });

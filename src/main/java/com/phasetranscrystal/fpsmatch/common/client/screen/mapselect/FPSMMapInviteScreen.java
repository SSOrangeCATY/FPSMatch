...
public class FPSMMapInviteScreen extends Screen implements FPSMMapDetailChildScreen {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (detail.availableInviteTargets().isEmpty()) {
            graphics.centeredText(font, Component.translatable("gui.fpsm.map_select.invite.empty"), width / 2, LIST_TOP + 32, 0xFFAAAAAA);
        }
        int y = LIST_TOP;
        for (int i = 0; i < Math.min(detail.availableInviteTargets().size(), visibleRows()); i++) {
            MapRoomPlayerInfo player = detail.availableInviteTargets().get(i);
            graphics.text(font, Component.literal(player.name()), left, y + 8, 0xFFE6F2FF, false);
            graphics.text(font, Component.translatable("gui.fpsm.map_select.online"), left + 160, y + 8, 0xFF74E084, false);
            y += ROW_HEIGHT;
        }

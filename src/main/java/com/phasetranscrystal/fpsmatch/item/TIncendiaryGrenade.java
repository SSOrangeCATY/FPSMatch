...
public class TIncendiaryGrenade extends Item {
    public TIncendiaryGrenade(Properties pProperties) {
        super(pProperties);
    }

    @SubscribeEvent
    public static void onPlayerInteract(PlayerInteractEvent.LeftClickEmpty event) {
        if (event.getEntity().getItemInHand(InteractionHand.MAIN_HAND).getItem() instanceof TIncendiaryGrenade grenade && !event.getEntity().getCooldowns().isOnCooldown(grenade)) {
    ...
    public static void onPlayerInteract(PlayerInteractEvent.LeftClickBlock event) {
        ...
        if (event.getEntity().getItemInHand(InteractionHand.MAIN_HAND).getItem() instanceof TIncendiaryGrenade grenade) {
    ...
    public ItemStack throwIncendiaryGrenade(Player pPlayer, Level pLevel, InteractionHand pHand, float velocity, float inaccuracy) {
        ...
        if (!pLevel.isClientSide) {
            TIncendiaryGrenadeEntity shell = new TIncendiaryGrenadeEntity(pPlayer, pLevel);
